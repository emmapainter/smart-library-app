...
struct ProgressBookCover: View {
    ...
    var body: some View {
        if let book = user.readingBooks.first(where: {$0 == readingBook}) {
            GeometryReader { metrics in
                ZStack {
                    AsyncImage(
                        url: book.book.getImageUrl(size: .large),
                        content: { image in
                            image.resizable()
            ...
            .onAppear {
                if let totalPages = book.book.pages {
                    progress = CGFloat(book.bookmark.currentPageNumber) / CGFloat(totalPages)
                }
            }
            .onChange(of: book.bookmark.currentPageNumber) { pageNumber in
                if let totalPages = book.book.pages {
                    progress = CGFloat(pageNumber) / CGFloat(totalPages)
                }
            }
        }
    }
